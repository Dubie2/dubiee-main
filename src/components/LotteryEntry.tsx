...
export function LotteryEntry() {
  ...
  const submit = async () => {
    ...
    const res = await addTickets(all, name.trim(), `${dial}${phone.replace(/\D/g, "")}`);
    setLoading(false);
    
    if (res.added === 0) {
      const reason = res.failed?.[0]?.reason || "الكود غير متاح أو مستخدم مسبقاً";
      setError(`${reason}: ${res.duplicates.join("، ")}`);
      return;
    }
    setResult(res.total);
    setChips([]);
